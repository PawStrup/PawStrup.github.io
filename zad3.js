...
const filterFunction = (array, phrase) => {
  ...
  if (typeof phrase === "number" && phrase.toString().length <= 2) {
    return "number too short";
  }

  const arrayOfElements = [];
  //dla każdego elementu array odpalamy funkcję makeArrayFromObjects i otrzymujemy jeden wielki array stringów i numberów
  array.forEach((element) => {
    makeArrayFromObjects(element, arrayOfElements);
  });

  //definiujemy regular expression
  const regex = new RegExp(phrase);

  //filtrujemy array i zwracamy przefiltrowany array
  const filteredArray = arrayOfElements.filter((element) => {
    return regex.exec(element);
  });
  return filteredArray;
};

//funckja makeArrayFromObjects tworzy jeden array stringów i numberów z arraya obiektów i ich zagnieżdżeń

const makeArrayFromObjects = (element, arr) => {
  //sprawdzamy czy dany element jest stringiem lub numberem, jeśli tak pushujemy do arraya
  if (typeof element === "string" || typeof element === "number") {
    arr.push(element);
  }
  //jeśli jest obiektem, tworzymy z niego array
  if (typeof element === "object") {
    let arrayOfObjects = Object.values(element);
    //następnie dla każdego elementu tego arraya znów sprawdzamy jego typ wykorzystując rekurencje
    arrayOfObjects.forEach((element1) => {
      makeArrayFromObjects(element1, arr);
    });
  }
  return arr;
};

console.log(filterFunction(data, "ers"));
